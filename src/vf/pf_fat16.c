...
pf_s32 VFiPFFAT16_ReadFATEntry(PF_VOLUME* p_vol, pf_u32 cluster, pf_u32* p_value) {
    ...
    if (((cluster < 2U) || (cluster >= (p_vol->bpb.num_clusters + 2))) && (cluster != 0) && (cluster != 1)) {
        *p_value = -1U;
        return 0xE;
    }
    fat_offset = cluster * 2;
    fat_sector = (pf_u16)(p_vol->bpb.active_FAT_sector + ((cluster * 2) >> p_vol->bpb.log2_bytes_per_sector));
    ...
    do {
        err = VFiPFSEC_ReadFAT(p_vol, buf, fat_sector, offset_in_sector, 2U);
...
pf_s32 VFiPFFAT16_WriteFATEntry(PF_VOLUME* p_vol, pf_u32 cluster, pf_u32 value) {
    ...
    fat_offset = cluster * 2;
    fat_sector = p_vol->bpb.active_FAT_sector + ((cluster * 2) >> p_vol->bpb.log2_bytes_per_sector);
    offset_in_sector = fat_offset & (p_vol->bpb.bytes_per_sector - 1);
    *(pf_u16*)buf = PF_SWAP_16(value);
    err = VFiPFSEC_WriteFAT(p_vol, buf, fat_sector, offset_in_sector, 2U);
    return err;
}
...
pf_s32 VFiPFFAT16_WriteFATEntryPage(PF_VOLUME* p_vol, pf_u32 cluster, pf_u32 value, PF_CACHE_PAGE** pp_page) {
    ...
    (*pp_page)->p_buf[offset_in_sector + 1] = (value >> 8U);
