...
static pf_s32 VFiPFDIR_p_mkdir(PF_VOLUME* p_vol /* r29 */, PF_STR* p_path /* r1+0x8 */, pf_u32 option /* r1+0xC */, PF_DTA* p_dta /* r31 */) {
    ...
    pf_u8 num_entry_LFNs;   // r1+0x11
    pf_u8 sum;              // r1+0x10
    pf_u32 OldSector;       // r1+0x34
    pf_s32 pozIdx;          // r1+0x30
    pf_u32 NextChain[2];    // r1+0x48
    pf_s32 err;             // r30
    pf_u16 filename_size;   // r1+0x12
    PF_FFD ffd;             // r1+0xA0
    PF_FAT_HINT fat_hint;   // r1+0x50
    pf_u32 SVSect;          // r1+0x2C
    pf_u32 sector;          // r1+0x28
    ...
    err = VFiPFENT_ITER_GetEntryOfPath(&iter, &entry_dir, p_vol, &dir_path, 1U);
    if (err != 0) {
        return err;
    }
    ...
    filename_size = VFiPFSTR_StrNumChar(&filename, 1U);
    if (filename_size > 0xFFU) {
        return 1;
    }
    if ((filename_size + entry_dir.path_len) > 0x103U) {
        return 2;
    }
    if (VFiPFSTR_GetCodeMode(&filename) == 2) {
        VFiPFPATH_transformFromUnicodeToNormal((pf_s8*)&tmp_local, (pf_u16*)VFiPFSTR_GetStrPos(&filename, 1U));
    }
    ...
    VFipf_memcpy(&entry, &entry_dir, 0x240U);
    err = VFiPFENT_ITER_FindEntry(&iter, &entry, &filename, 0x10U, 0U, &is_found, 1U);
    if (err != 0) {
        return err;
    }
    if (is_found != 0) {
    ...
    if (err != 0) {
        ...
        if (VFiPFSTR_GetCodeMode(&filename) == 1) {
            filename_size = VFiPFPATH_transformInUnicode(entry.long_name, VFiPFSTR_GetStrPos(&filename, 1U));
        } else {
            VFipf_w_strcpy(entry.long_name, (pf_u16*)VFiPFSTR_GetStrPos(&filename, 1U));
        }
    } else {
        *entry.long_name = 0;
    }
    ...
    if ((*entry.long_name != 0) && ((entry.small_letter_flag & 0x18) == 0)) {
        ...
        err = VFiPFENT_allocateEntryPos(&entry, 1U, &ffd, (pf_u32*)&NextChain, &filename, &position);
        if (err != 0) {
            return err;
        }
    }
    ...
    err = VFiPFFAT_GetSectorSpecified(&ffd, 0U, 1U, &sector);
    if (err != 0) {
        return err;
    }
    if (sector == -1) {
    ...
    for (spc_cnt = p_vol->bpb.sectors_per_cluster; spc_cnt != 0; spc_cnt--) {
        if (spc_cnt == 1U) {
            ...
            VFiPFENT_SetDotDotEntry(p_page->p_buf + 0x20);
            if (entry_dir.entry_sector == -1) {
                entry.start_cluster = 0;
            } else {
                entry.start_cluster = entry_dir.start_cluster;
            }
            VFiPFENT_StoreEntryNumericFieldsToBuf(p_page->p_buf + 0x20, &entry);
        }
        err = VFiPFSEC_WriteData(p_vol, p_page->p_buf, (sector + spc_cnt) - 1, 0U, p_vol->bpb.bytes_per_sector, &success_size, 0U);
        if (err == 0) {
    ...
    if ((entry.small_letter_flag & 0x18) == 0) {
        for (i = num_entry_LFNs; i >= 1; i--) {
            VFiPFENT_storeLFNEntryFieldsToBuf((pf_u8*)&buf, &entry, (pf_u8)i, sum, i == num_entry_LFNs ? 1 : 0);
            err = VFiPFSEC_WriteData(p_vol, (pf_u8*)&buf, OldSector, (pf_u16)entry.entry_offset, 0x20, &success_size, 0);
            if (err != 0) {
                return err;
            }
    ...
    err = VFiPFENT_updateEntry(&entry, 0U);
    if (err != 0) {
        return err;
    }
    if (p_dta != PF_NULL) {
...
pf_s32 VFiPFDIR_mkdir(PF_STR* p_path_str) {
    ...
    if (VFiPFSTR_StrNCmp(p_path_str, (pf_s8*)"\0", 1U, 0, 1) == 0) {
        return VFipf_vol_set.last_error = 0xA;
    }
