...
static pf_s32 VFiPFENT_RecalcEntryIterator(PF_ENT_ITER* p_iter, pf_u32 may_allocate) {
    ...
    if (file_sector_index != p_iter->file_sector_index) {
        ...
        if ((p_iter->sector == -1U) && (may_allocate != 0)) {
            ...
            if ((p_iter->sector != -1U) && (is_initialize != -1U)) {
                err = VFiPFCACHE_AllocateDataPage(p_iter->p_vol, -1U, &p_page);
                if (err != 0) {
                    return err;
                }
                ...
                for (num_sectors = 0; num_sectors < p_iter->p_vol->bpb.sectors_per_cluster; num_sectors++) {
                    err = VFiPFSEC_WriteData(p_iter->p_vol, p_page->p_buf, p_iter->sector + num_sectors, 0U, p_iter->p_vol->bpb.bytes_per_sector,
                                             &success_size, 0U);
                    if (err != 0) {
...
static pf_u32 VFiPFENT_ITER_IsAtPhysicalEnd(PF_ENT_ITER* p_iter) {
    if (p_iter->sector == -1U) {
        return 1U;
    }
    return 0U;
...
// DEBUG NON MATCHING
static pf_s32 VFiPFENT_ITER_FindCluster(PF_ENT_ITER* p_iter /* r30 */, PF_DIR_ENT* p_ent /* r31 */, pf_u32 cluster /* r24 */,
                                        pf_u32* p_is_found /* r26 */) {
    pf_u8 attr;          // r28
    pf_s32 err;          // r29
    pf_s32 lengthName;   // r27
    pf_s8 filename[13];  // r1+0x8
    ...
    *p_is_found = 0;
    ...
    for (; VFiPFENT_ITER_IsAtLogicalEnd(p_iter) == PF_FALSE; err = VFiPFENT_ITER_Advance(p_iter, 0U)) {
        ...
        if (cluster != (((pf_u16)((pf_u8)(((pf_u16*)p_iter->buf)[10] >> 8U) | ((((pf_u16*)p_iter->buf)[10] << 8) & 0xFF00)) << 0x10) |
                        (pf_u16)((pf_u8)(((pf_u16*)p_iter->buf)[13] >> 8U) | ((((pf_u16*)p_iter->buf)[13] << 8) & 0xFF00)))) {
            p_ent->num_entry_LFNs = 0;
            p_ent->long_name[0] = 0;
            continue;
        }
...
static pf_s32 VFiPFENT_ITER_DoFindEntry(PF_ENT_ITER* p_iter, PF_DIR_ENT* p_ent, PF_STR* p_pattern, pf_u8 attr_required, pf_u8 attr_unwanted,
    ...
    *p_is_found = 0;
    ...
    if ((VFiPFSTR_StrNCmp(p_pattern, (pf_s8*)".\\", 1U, 0, 2U) == 0) || (VFiPFSTR_StrCmp(p_pattern, (pf_s8*)".") == 0)) {
...
static pf_s32 VFiPFENT_ITER_DoGetEntry(PF_ENT_ITER* p_iter, PF_DIR_ENT* p_ent, PF_VOLUME* p_vol, PF_STR* p_path, pf_u32 wildcard,
    ...
    for (err = VFiPFPATH_GetNextTokenOfPath(&token, wildcard); token.p_head != 0; err = VFiPFPATH_GetNextTokenOfPath(&token, wildcard)) {
        ...
        if ((no_look_last_token != 0) && (VFiPFSTR_StrNCmp(&token, (pf_s8*)"\0", 2U, 0, 1U) == 0)) {
            break;
        }
        ...
        err = VFiPFENT_ITER_DoFindEntry(p_iter, p_ent, &token, 0x7FU, 0U, &is_found, 1U);
        if (err != 0) {
            return err;
        }
        if (is_found == 0) {
        ...
        if (wildcard == 0) {
            ...
            if (((p_ent->attr & 0x10) == 0) || (p_ent->short_name[0] != 0x2E)) {
                p_ent->path_len = (VFiPFSTR_StrNumChar(&token, 1U) + 1) + p_ent->path_len;
...
pf_u32 VFiPFENT_ITER_IsAtLogicalEnd(PF_ENT_ITER* p_iter) {
    ...
    if ((VFiPFENT_ITER_IsAtPhysicalEnd(p_iter) != 0) || (p_iter->buf[0] == 0) || (p_iter->index >= 0xF423F)) {
        ...
        err = VFiPFENT_RecalcEntryIterator(p_iter, 0U);
        if (err != 0) {
...
pf_s32 VFiPFENT_ITER_GetEntryOfPath(PF_ENT_ITER* p_iter, PF_DIR_ENT* p_ent, PF_VOLUME* p_vol, PF_STR* p_path, pf_u32 no_look_last_token) {
    return VFiPFENT_ITER_DoGetEntry(p_iter, p_ent, p_vol, p_path, 0U, no_look_last_token);
}
