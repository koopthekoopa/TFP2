...
static void VFiPFFILE_Cursor_SetPosition(PF_FILE* p_file, pf_u32 pos) {
    ...
    post_sec_off = (pos >> p_vol->bpb.log2_bytes_per_sector) + ((pos & (p_vol->bpb.bytes_per_sector - 1)) != 0 ? 1 : 0);
    if (pre_sec_off != post_sec_off) {
        p_file->cursor.sector = -1U;
    }
    p_file->cursor.position = pos;
    VFiPFFILE_Cursor_Recalc(p_file);
}

static pf_u32 VFiPFFILE_Cursor_AdvanceToRead(PF_FILE* p_file, pf_u32 n, pf_u32 sector) {
    pf_u32 res;
    PF_VOLUME* p_vol;
    pf_u32 wk_sector;
...
static void VFiPFFILE_Cursor_Initialize(PF_FILE* p_file) {
    p_file->cursor.sector = -1U;
    VFiPFFILE_Cursor_SetPosition(p_file, 0U);
}

static pf_u32 VFiPFFILE_Cursor_IsOutOfFile(PF_FILE* p_file) {
    if (p_file->cursor.position >= p_file->p_sfd->dir_entry.file_size) {
        return 1U;
    }
    return 0U;
...
static pf_s32 VFiPFFILE_Cursor_ReadHeadSector(PF_VOLUME* p_vol, PF_FILE* p_file, pf_u8* p_buf, pf_u32 size, pf_u32* p_size_read) {
    ...
    *p_size_read = 0;
    ...
    if ((p_file->cursor.sector) == -1U) {
        err = VFiPFFAT_GetSectorSpecified(&p_file->p_sfd->ffd, p_file->cursor.file_sector_index, 0U, &p_file->cursor.sector);
        if (err != 0) {
            return err;
        }
        if ((p_file->cursor.sector) == -1U) {
...
static pf_s32 VFiPFFILE_Cursor_WriteHeadSector(PF_VOLUME* p_vol, PF_FILE* p_file, pf_u8* p_buf, pf_u32 size, pf_u32* p_size_write) {
    ...
    *p_size_write = 0;
    ...
    err = VFiPFSEC_WriteData(p_vol, p_buf, p_file->cursor.sector, p_file->cursor.offset_in_sector, size, &success_size, 1U);
    if ((err != 0) && (success_size == 0)) {
        return err;
    }
...
static pf_s32 VFiPFFILE_Cursor_WriteTailSector(PF_VOLUME* p_vol, PF_FILE* p_file, pf_u8* p_buf, pf_u32 size, pf_u32 append_size,
    ...
    *p_size_write = 0;
    ...
    } else if ((p_file->cursor.sector) == -1U) {
        err = VFiPFFAT_GetSectorSpecified(&p_file->p_sfd->ffd, p_file->cursor.file_sector_index, 0U, &p_file->cursor.sector);
        if (err != 0) {
            return err;
        }
...
static pf_s32 VFiPFFILE_Cursor_Write_Overwrite(PF_VOLUME* p_vol, PF_FILE* p_file, pf_u8* p_buf, pf_u32 size, pf_u32* p_size_write) {
    ...
    *p_size_write = 0;
    ...
    if ((num_sector == 0) || ((p_file->cursor.sector) == -1U)) {
        VFiPFFILE_Cursor_MoveToEnd(p_file);
        return 0x1C;
    }
...
void VFiPFFILE_Cursor_MoveToClusterEnd(PF_FILE* p_file, pf_u32 size) {
    ...
    if (cluster > (-1U >> (p_vol->bpb.log2_bytes_per_sector + p_vol->bpb.log2_sectors_per_cluster))) {
        VFiPFFILE_Cursor_SetPosition(p_file, -1U);
        return;
    }
...
static PF_SFD* VFiPFFILE_GetSFD(PF_VOLUME* p_vol, PF_DIR_ENT* p_ent) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (((PF_VOLUME*)p_ent->p_vol == (PF_VOLUME*)p_vol->sfds[i].dir_entry.p_vol) &&
            (p_ent->entry_sector == p_vol->sfds[i].dir_entry.entry_sector) && (p_ent->entry_offset == p_vol->sfds[i].dir_entry.entry_offset)) {
...
static pf_s32 VFiPFFILE_createEmptyFile(PF_VOLUME* p_vol, PF_DIR_ENT* p_file_ent, PF_STR* p_filename, pf_u32 namelength) {
    ...
    if (err != 0) {
        ...
        if (VFiPFSTR_GetCodeMode(p_filename) == 1) {
            VFiPFPATH_transformInUnicode(p_file_ent->long_name, VFiPFSTR_GetStrPos(p_filename, 1U));
        } else {
            VFipf_w_strcpy(p_file_ent->long_name, (pf_u16*)VFiPFSTR_GetStrPos(p_filename, 1U));
        }
    } else {
    ...
    if ((p_file_ent->long_name[0] != 0) && ((p_file_ent->small_letter_flag & 0x18) == 0)) {
        ...
        for (i = num_entry_LFNs; i >= 1; i--) {
            ...
            err = VFiPFSEC_WriteData(p_vol, (pf_u8*)&buf, OldSector, (pf_u16)p_file_ent->entry_offset, 0x20U, &success_size, 0U);
            if (err != 0) {
                return err;
            }
        ...
        err = VFiPFENT_allocateEntry(p_file_ent, 1U, &ffd, (pf_u32*)&NextChain, p_filename);
        if (err != 0) {
            if (err == 8) {
                return err;
            }
    ...
    return VFiPFENT_updateEntry(p_file_ent, 1U);
}

static void VFiPFFILE_EmptyFile(PF_FFD* p_ffd, PF_DIR_ENT* p_ent) {
    if ((p_ent->start_cluster >= 2U) && ((p_ent->start_cluster) != -1U) && (p_ent->file_size != 0)) {
        VFiPFFAT_FreeChain(p_ffd, p_ent->start_cluster, -1U, p_ent->file_size);
    }
...
static pf_s32 VFiPFFILE_p_remove(PF_VOLUME* p_vol, PF_STR* p_path_str) {
    ...
    err = VFiPFENT_ITER_GetEntryOfPath(&iter, &ent, p_vol, p_path_str, 0U);
    if (err != 0) {
        return err;
    }
    p_vol = ent.p_vol;
...
static pf_s32 VFiPFFILE_p_fopen(PF_VOLUME* p_vol, PF_STR* p_path_str, pf_s32 mode, PF_FILE** pp_file) {
    ...
    *pp_file = PF_NULL;
    err = VFiPFENT_ITER_GetEntryOfPath(&iter, &ent, p_vol, p_path_str, 1U);
    if (err != 0) {
        return err;
    }
    if ((ent.attr & 0x10) == 0) {
    ...
    if (VFiPFSTR_GetCodeMode(&file_str) == 2) {
        VFiPFPATH_transformFromUnicodeToNormal((pf_s8*)&tmp_local, (pf_u16*)VFiPFSTR_GetStrPos(&file_str, 1U));
    }
    VFiPFSTR_SetLocalStr(&file_str, (pf_s8*)&tmp_local);
    if ((mode & 2) != 0) {
        VFiPFFAT_InitFFD(&ffd, &hint, ent.p_vol, &ent.start_cluster);
        err = VFiPFENT_findEntry(&ffd, &ent, 0U, &file_str, 0x7FU, 0U);
        if (err != 0) {
            return 3;
        }
...
static pf_s32 VFiPFFILE_p_fwrite(PF_VOLUME* p_vol, pf_u8* p_buf, pf_u32 size, pf_u32 count, PF_FILE* p_file, pf_u32* p_count_written) {
    ...
    *p_count_written = 0;
    ...
    if (((p_file->p_sfd->lock.mode & 3) != 0) &&
        (((p_file->p_sfd->lock.mode & 1) != 0) || (((p_file->p_sfd->lock.mode & 2) != 0) && (p_file->lock_count == 0)))) {
        return 0x19;
    }
    if ((p_file->open_mode & 4) != 0) {
        VFiPFFILE_Cursor_MoveToEnd(p_file);
...
pf_s32 VFiPFFILE_GetOpenedFile(PF_DIR_ENT* p_ent, PF_DIR_ENT** pp_open_ent) {
    ...
    *pp_open_ent = PF_NULL;
    for (i = 0; i < 5; i++) {
        if (((p_ent->p_vol->sfds[i].stat & 1) != 0) && ((p_ent->p_vol->sfds[i].stat & 2) != 0) &&
            ((PF_VOLUME*)p_ent->p_vol == (PF_VOLUME*)p_ent->p_vol->sfds[i].dir_entry.p_vol) &&
...
pf_u32 VFiPFFILE_IsOpened(PF_DIR_ENT* p_ent) {
...
pf_s32 VFiPFFILE_DoRemoveFile(PF_DIR_ENT* p_ent, PF_ENT_ITER* p_iter) {
    ...
    err = VFiPFFAT_FreeChain(&p_iter->ffd, start_cluster, -1U, p_ent->file_size);
    if (err != 0) {
        return err;
    }
    return err;
...
pf_s32 VFiPFFILE_fclose(PF_FILE* p_file) {
    ...
    if ((p_file == PF_NULL) || ((PF_SFD*)p_file->p_sfd == PF_NULL) || ((p_file->stat & 1) == 0) || ((p_file->p_sfd->stat & 1) == 0) ||
    ...
    if (((p_file->p_sfd->num_handlers - 1) <= 0) && ((p_file->p_sfd->dir_entry.attr & 0x19) == 0) && (VFiPFDRV_IsWProtected(p_vol) == 0) &&
        ...
        err = VFiPFENT_updateEntry(&p_file->p_sfd->dir_entry, 1U);
    }
    if (p_file->lock_count != 0) {
        if ((p_file->p_sfd->lock.mode & 1) != 0) {
            ...
            p_file->p_sfd->lock.mode &= (pf_u16)~3;
        } else if ((PF_FILE*)p_file->p_sfd->lock.owner != p_file) {
            err = 0x19;
        } else {
...
pf_s32 VFiPFFILE_fread(pf_u8* p_buf, pf_u32 size, pf_u32 count, PF_FILE* p_file, pf_u32* p_count_read) {
    ...
    *p_count_read = 0;
    ...
    p_vol = p_file == PF_NULL ? PF_NULL : p_file->p_sfd->dir_entry.p_vol;
    if (p_vol == PF_NULL) {
        return p_file->last_error = p_file->p_sfd->ffd.p_vol->last_error = VFipf_vol_set.last_error = 0x26;
    }
    err = VFiPFVOL_CheckForRead(&*p_vol);
    if (err != 0) {
        return p_vol->last_error = VFipf_vol_set.last_error = err;
    ...
    if ((p_file == PF_NULL) || ((PF_SFD*)p_file->p_sfd == PF_NULL) || ((p_file->stat & 1) == 0) || ((p_file->p_sfd->stat & 1) == 0) ||
        ((p_file->p_sfd->stat & 2) == 0)) {
        return p_vol->last_error = VFipf_vol_set.last_error = 0x26;
    }
    if (((p_file->p_sfd->stat & 1) == 0) || ((p_file->p_sfd->stat & 2) == 0)) {
        return p_vol->last_error = VFipf_vol_set.last_error = 0x26;
    }
    if ((p_buf == PF_NULL) || (count == 0) || (size == 0)) {
        return p_file->last_error = p_file->p_sfd->ffd.p_vol->last_error = VFipf_vol_set.last_error = 0xA;
    }
    p_vol->cache.signature = p_file;
...
pf_s32 VFiPFFILE_fseek(PF_FILE* p_file, pf_s32 lOffset, pf_s32 nOrigin) {
    ...
    p_vol = p_file == PF_NULL ? PF_NULL : p_file->p_sfd->dir_entry.p_vol;
    if (p_vol == PF_NULL) {
        return VFipf_vol_set.last_error = 0x26;
    }
    err = VFiPFVOL_CheckForRead(&*p_vol);
    if (err != 0) {
        return p_vol->last_error = VFipf_vol_set.last_error = err;
