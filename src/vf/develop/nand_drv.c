...
s32 VFi_NandOpenSp(const char* path, void* info, u8 accType, u32 i_handleIdx) {
    ...
    info_p = info;
    if (i_handleIdx < 26) {
        VFi_NANDOpen tmpOpen;
        tmpOpen = l_nandFunc[i_handleIdx].open;
        if (tmpOpen != NULL) {
...
static s32 _CreateNANDFileWithDir(const char* i_path_p, u8 i_perm, u32 i_handleIdx) {
    s32 nandErr = 0;
    nandErr = VFi_NandCreateSp(i_path_p, i_perm, 0U, i_handleIdx);
    return (s16)nandErr;
}
...
static s32 A32_NANDRead(NANDFileInfo* i_fileInfo_p, void* i_buf, u32 i_size) {
    ...
    if (size_1st == 0) {
        ...
        NANDError = VFi_NandSeek(i_fileInfo_p, -size_3rd, 1);
        if (NANDError < 0) {
            return NANDError;
        }
        if (size_2nd != 0) {
...
static s32 _CreatePrfFile(u32 i_fileSize, char* i_fullpath_p, u16 i_version, u8 i_perm, u32 i_handleIdx) {
    ...
    if (nandError != 0) {
        return (s16)nandError;  // i dont get the cast
    }

    nandError = VFi_NandOpenSp(i_fullpath_p, &fileInfo, 2U, i_handleIdx);
    if (nandError != 0) {
        return (s16)nandError;  // again i dont get it
    }

    dCommon_CopyPrfFileHeader(&work, i_fileSize, i_version, 0U);
    nandError = VFi_NandWrite(&fileInfo, &work, 0x20U);
    restSize -= 0x20;
    ...
    VFipf_memset(&work, 0, 0x200U);

    while (restSize >= 0x200U) {
        nandError = VFi_NandWrite(&fileInfo, &work, 0x200U);
        if (nandError < 0) {
            VFi_NandClose(&fileInfo);
            VFi_NandDeleteSp(i_fullpath_p, i_handleIdx);
            return nandError;
...
s32 VFi_NandFlushNANDFromHandleIdx(s32 i_handleIdx, int i_setLastDeviceError) {
    ...
    NANDError = 0;
    drive_p = VFSysGetDriveP(i_handleIdx);
    handle_p = VFSysGetHandleP(i_handleIdx);
    if ((handle_p != NULL) && (handle_p->device_p != NULL) && (handle_p->device_p->sync_mode == 1U)) {
        return 0;
    }
    if (drive_p != NULL) {
        ...
        NANDError = VFi_NandOpenSp(sys_name_p, fileInfo_p, 3U, i_handleIdx);
        if (NANDError < 0) {
            if (i_setLastDeviceError != 0) {
                dCommon_setLastDeviceErrorToDisk2(i_handleIdx, NANDError);
            }
...
static s32 _MountPrfFile(PDM_DISK* p_disk, char* i_fullpath_p) {
    ...
    nandError = VFi_NandOpenSp(i_fullpath_p, fileInfo_p, 1U, handleIdx);
    if (nandError == 0) {
        VFipf_memset(&header, 0, 0x20U);
        nandError = A32_NANDRead(fileInfo_p, &header, 0x20U);
        if (nandError < 0) {
        ...
        if (dCommon_IsPrfFile(&header) == 0) {
            ...
            dCommon_setFileSizeToDisk(p_disk, header.fileSize[3] |
                                                  ((header.fileSize[2] << 8) | ((header.fileSize[0] << 0x18) | (header.fileSize[1] << 0x10))));
            fileSize = dCommon_getFileSizeFromDisk(p_disk);
            dataSize = (fileSize - 0x20);
            SPU = ((dataSize + 1) >> 9U);
            fatType = dCommon_GetNiceFatType(NULL, SPU, 1U, 0x200U);
...
static s32 nanddrv_BuildUpFSInfoSector(u8* buf) {
    if (dCommon_MakeFsInfoSec(buf, 0x200U) == 0) {
        return 0;
    }
    return -0x15;
}
...
static s32 nanddrv_BuildUpBootSector(PDM_DISK* p_disk, u8* buf, PDM_FAT_TYPE* type) {
    ...
    realDisk_p = &VFipdm_disk_set.disk[PDM_DISK_GET_NO(p_disk)];
    drvSPU = realDisk_p->disk_info.total_sectors;
    dCommon_MakeBootSector(buf, type, drvSPU, 1U, 0x200U, 0xEBU, 0x90U, 0x3FU, dCommon_getResvSecNumFromDisk(p_disk),
...
static s32 nanddrv_pread(PDM_DISK* p_disk, u8* p_buf, u32 block, u32 num_blocks, u32* p_num_success) {
    *p_num_success = 0;
    ...
    return nanddrv_physical_read(num_blocks, p_buf, block, 0x200U, p_num_success, p_disk);
}
...
static s32 nanddrv_pwrite(PDM_DISK* p_disk, const u8* p_buf, u32 block, u32 num_blocks, u32* p_num_success) {
    *p_num_success = 0;
    ...
    return nanddrv_physical_write(num_blocks, p_buf, block, 0x200U, p_num_success, p_disk);
}
...
static s32 nanddrv_get_disk_info(PDM_DISK* p_disk, PDM_DISK_INFO* p_disk_info) {
    ...
    fileSize = dCommon_getFileSizeFromDisk(p_disk);
    dataSize = fileSize - 0x20;
    p_disk_info->total_sectors = (dataSize + 1) >> 9U;
...
static s32 nanddrv_physical_read(u32 num_blocks, u8* buf, u32 block, u32 bps, u32* p_num_success, PDM_DISK* p_disk) {
    ...
    nandError = VFi_NandSeek(fileInfo_p, (s32)offset, 0);
    if (nandError == offset) {
