...
static int l_vfsys_dev_table_init = 0;
...
static void VFSys_free_cache(VFSys_cache* i_cache_p) {
    if (i_cache_p != NULL) {
        if ((PF_CACHE_PAGE*)i_cache_p->pf_cache_page_p != NULL) {
...
static void VFSys_create_cache(VFSys_handle* i_handle_p, u32 i_pages, MEMiHeapHead* i_heap_handle) {
    if (i_handle_p != NULL) {
        ...
        if ((cache_p->pf_cache_page_p == NULL) && (cache_p->pf_cache_buf_p == NULL)) {
            u32 cache_page_size;
            u32 cache_buf_size;
            cache_page_size = pages_2 * 0x28;
            cache_buf_size = pages_2 << 9;
            cache_p->pf_cache_page_p = VFSys_alloc_common(cache_page_size, 4U, i_heap_handle);
            cache_p->pf_cache_buf_p = VFSys_alloc_common(cache_buf_size, 4U, i_heap_handle);
            if ((cache_p->pf_cache_page_p != NULL) && (cache_p->pf_cache_buf_p != NULL)) {
...
static void VFSys_alloc_cache(VFSys_handle* i_handle_p, void* i_cache_heap_p, u32 i_cache_size) {
    if (i_handle_p != NULL) {
        u32 pages;
        MEMiHeapHead* heap_handle;
        pages = 8;
        heap_handle = l_vfsys_exp_heap_handle;
        if ((i_cache_heap_p != NULL) && (i_cache_size > 0x2800U)) {
...
static void VFSys_destroy_cache(VFSys_handle* i_handle_p) {
    if (i_handle_p != NULL) {
        ...
        VFSys_free_cache(cache_p);
        if ((heap_handle != l_vfsys_exp_heap_handle) && (heap_handle != 0)) {
...
static void VFSys_destroy_cache_all() {
    ...
    if (handle_p != NULL) {
        VFSys_handle* handle_end_p;

        for (handle_end_p = &handle_p[l_vfsys_vol_max]; handle_p != handle_end_p; handle_p++) {
...
static void VFSys_clear_handle(VFSys_handle* o_handle_p) {
    VFipf_memset(o_handle_p, 0, 0x140U);
...
static void VFSys_create_handle_table(u32 i_max) {
    if (l_sys_handle_table_p == NULL) {
        l_sys_handle_table_p = VFSys_alloc(i_max * 0x140, 0x20U);
    }
}
...
VFSys_handle* VFSys_file_p_2_handle_p(PF_FILE* i_file_p) {
    ...
    if ((i_file_p != NULL) && (i_file_p->p_sfd != NULL)) {
        PF_VOLUME* vol_p;
        vol_p = i_file_p->p_sfd->ffd.p_vol;
...
VFSys_deviceTblEntry* l_vfsys_dev_table[26];
...
static void VFSys_create_device_table(u32 i_max) {
    if (l_vfsys_dev_table_init == 0) {
        ...
        i = 0;
        for (; i < i_max; i++) {
            l_vfsys_dev_table[i] = VFSys_alloc(0xA0U, 0x20U);
        }
...
void VFSysInit(void* i_heap_start_address_p, u32 i_size) {
    ...
    l_vfsys_vol_max = vol_max > 26 ? 26 : vol_max;
...
static void VFSys_set_device_err_info(VFSys_handle* o_handle_p, s32 i_err) {
    if (o_handle_p != NULL) {
        PDM_DISK* pf_disk_p;
        pf_disk_p = o_handle_p->drive.pf_disk_p;
        if (pf_disk_p != NULL) {
...
static PDM_INIT_DISK l_dev_init_info_table[26];
...
s32 VFSysSetDeviceNANDFlash(s32* o_idx_p, void* i_cache_heap_p, u32 i_cache_size) {
    ...
    *o_idx_p = -1;
    if (handle_p == NULL) {
        ...
        VFipf_memcpy(init_info_p, &l_dev_nandflash_init_info, 8U);
        init_info_p->ui_ext = idx;
        err = VFipdm_open_disk(init_info_p, &handle_p->drive.pf_disk_p);
        if (err == 0) {
            VFSys_deviceTblEntry* uni_p;
            VFSys_deviceBase* nandflash_p;
...
s32 VFSysUnsetDevice(s32 i_handle_idx) {
    ...
    handle_p = VFSysGetHandleP(i_handle_idx);
    err = 0;
    if ((handle_p == NULL) || (handle_p->drive.pf_disk_p == 0)) {
        return 0xB003;
    }
    VFSys_set_device_err_info(handle_p, 0);
...
static s32 VFSysCheckExistPrfFile_nandflash_sub(const char* i_prf_file_name_p, u32 i_handle_idx) {
    ...
    NANDError = NAND_RESULT_OK;
    NANDErrorLen = 0;
    ret = 0xB001;
    VFSys_change_separater(file_name, i_prf_file_name_p, 0x2F, 0x5C, 0xFF);
    NANDError = VFi_NandOpenSp(file_name, &FileInfo, 1U, i_handle_idx);
    if (NANDError == 0) {
        u32 length;
        length = 0U;
        NANDErrorLen = VFi_NandGetLength(&FileInfo, &length);
        NANDError = VFi_NandRead(&FileInfo, &header, 0x20U);
        if ((NANDError >= NAND_RESULT_OK) && (NANDErrorLen >= 0) && (dCommon_IsPrfFile(&header) != 0) &&
            (length == (header.fileSize[3] | ((header.fileSize[2] << 8) | ((header.fileSize[0] << 0x18) | (header.fileSize[1] << 0x10)))))) {
            ret = 0;
        } else {
            ret = 0xB006;
        }
...
static s32 VFSysCheckExistPrfFile_ram_sub(void* i_memory_p) {
    if (dCommon_IsPrfFile(i_memory_p) != 0) {
        return 0;
    }
    return 0xB006;
}
...
static s32 VFSysCheckExistPrfFile_dvd_sub(const char* i_prf_file_name_p) {
    ...
    if (DVDError != DVD_RESULT_OK) {
        ...
        if ((readBytes != 0) && (dCommon_IsPrfFile(&header) != 0) &&
            (dvdFileSize == (header.fileSize[3] | ((header.fileSize[2] << 8) | ((header.fileSize[0] << 0x18) | (header.fileSize[1] << 0x10)))))) {
            ret = 0;
        } else {
            ret = 0xB006;
        }
...
s32 VFSysCreatePrfFileNANDFlashEx(const char* i_prf_file_name_p, u32 i_file_size) {
    ...
    VFSys_change_separater(file_name, i_prf_file_name_p, 0x2F, 0x5C, 0xFF);
    if (VFSysCheckExistPrfFile_nandflash_sub(file_name, -0xBU) == 0xB001) {
        return NAND_CreatePrfFileEx(i_file_size, file_name, 0x100U);
    }
    return 0;
}
...
s32 VFSysMountDrv(s32 i_handle_idx, const char* i_prf_file_name_p, void* i_memory_p) {
    ...
    if (err == 0) {
        ...
        dCommon_setFileSizeToDisk(drive_p->pf_disk_p, 0x19000U);
        return 0;
    }
    return VFipf2_errnum();
}
...
s32 VFSysUnmountDrv(s32 i_handle_idx, u32 i_mode) {
    ...
    if (err == 0) {
        dCommon_setFileSizeToDisk(drive_p->pf_disk_p, 0U);
        dCommon_setLastDeviceErrorToDisk(&*drive_p->pf_disk_p, 0);
        drive_p->pf_drv.p_part = NULL;
        drive_p->pf_drv.cache = NULL;
        drive_p->file_p = NULL;
        VFipf_memset(drive_p->pf_filename, 0, 0xFFU);
        return 0;
    }
    return VFipf2_errnum();
}
...
s32 VFSysReadFile(u32* o_read_size_p, void* o_buf_p, u32 i_size, PF_FILE* i_file_p) {
    ...
    if (VFipf2_finfo(i_file_p, &info) == 0) {
        ...
        offset = i_file_p->cursor.position;
        ...
        VFSys_file_p_2_set_device_err_info(i_file_p, 0);
        if (read_size != 0) {
            if (VFipf2_fread(o_buf_p, read_size, 1U, i_file_p) == 1) {
                if (o_read_size_p != NULL) {
...
s32 VFSysWriteFile(void* i_buf_p, u32 i_size, PF_FILE* i_file_p) {
    VFSys_file_p_2_set_device_err_info(i_file_p, 0);
    if (VFipf2_fwrite(i_buf_p, i_size, 1U, i_file_p) == 1) {
        return 0;
    }
    return VFipf2_errnum();
}
