...
void dCommon_CopyPrfFileHeader(void* o_buff_p, u32 i_fileSize, unsigned short i_version, u8 i_volatile_memory) {
    ...
    Header.version[0] = (i_version >> 8U) & 0xFF;
...
int dCommon_ReadDummyBPB(u32 num_blocks, u8* buf, u32 block, u32* p_num_success, PDM_DISK* p_disk, s32* p_err, PDM_MAKE_BS makeBS,
                         PDM_MAKE_FS makeFS) {
    *p_err = 0;
...
int dCommon_WriteDummyBPB(u32 num_blocks, u32 block, u32* p_num_success, PDM_DISK* p_disk, s32* p_err) {
    *p_err = 0;
    if (block < dCommon_getResvSecNumFromDisk(p_disk)) {
        if (((s32)block == 0) || ((block == 1) && (dCommon_getFatTypeFromDisk(p_disk) == 2))) {
            *p_num_success = num_blocks;
            *p_err = 0;
            return 0;
        }
...
void dCommon_setLastDeviceErrorToDisk2(s32 i_handleIdx, s32 i_lastDevErr) {
    if ((i_handleIdx >= 0) && (i_handleIdx < 26)) {
...
static s32 dCommon_flush_from_handle_p(VFSys_handle* i_handle_p, int i_setLastDeviceError) {
    s32 err;
    ...
    if ((i_handle_p != NULL) && (i_handle_p->device_p != NULL)) {
        if (i_handle_p->device_p->type == 0) {
            int handle_idx;
            handle_idx = VFSysHandleP2Idx(i_handle_p);
            if (handle_idx != -1) {
...
s32 dCommon_FlushFromHandleIdx(int i_handle_idx, int i_setLastDeviceError) {
